//! This is an advanced example of stopwatch-with-ssd1331-and-interrupts.rs
//!
//! This example requires the `rt` feature to be enabled. For this example, you need stm32f4Discvoery and ssd1331 (SPI).
//!
...
fn main() -> ! {
    ...
    // Set up the LEDs. On the stm324discovery they are connected to pin PD12(Green) and PD14(Red).
    let mut led3 = gpiod.pd12.into_push_pull_output();
    let mut led4 = gpiod.pd14.into_push_pull_output();
    ...
    loop {
        ...
        Text::new(format_buf.as_str(), Point::new((100 / 2) - 1, 0))
            .into_styled(text_style)
            .draw(&mut disp)
            .unwrap();
...
fn draw_face() -> impl Iterator<Item = Pixel<Rgb565>> {
    let tic_len = 3.0;
...
fn draw_seconds_hand(seconds: u32) -> impl Iterator<Item = Pixel<Rgb565>> {
    ...
    // Basic line hand
    let hand = Line::new(CENTER, end).into_styled(PrimitiveStyle::with_stroke(Rgb565::WHITE, 1));
